      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-[#050505]/80 backdrop-blur-xl border-b border-white/5 py-4' : 'bg-transparent py-6'}`}>
        <div className="max-w-[1400px] mx-auto px-6 h-10 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            {/* Right side buttons (Auth + Mobile Menu) */}
            <div className="flex items-center gap-4">
              {/* Desktop-only Auth Section (if not logged in) */}
              {!user && (
              ...
              {/* User Profile - Visible on Mobile and Desktop */}
              {user && (
                <div className="relative group/avatar-btn">
                  <button
                    onClick={() => setIsProfileOpen(!isProfileOpen)}
                    className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#0a0a0a] border border-white/10 p-0.5 hover:border-trae-green/50 transition-all duration-300 overflow-hidden relative group"
                  >
                  ...
                  {/* Pulsing Status Dot */}
                  <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-[#050505] rounded-full flex items-center justify-center pointer-events-none">
                    <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-trae-green rounded-full shadow-[0_0_8px_rgba(16,185,129,0.8)] animate-pulse" />
                  </div>
                    ...
                    {isProfileOpen && (
                      ...
                      >
                        <div className="px-4 py-2 border-b border-white/5 mb-2">
                          <p className="text-sm font-bold text-white mb-0.5 truncate">{user.name}</p>
                          <p className="text-[10px] text-trae-green font-mono uppercase tracking-widest">{user.role}</p>
                        </div>
              ...
              <button
                ...
                className="lg:hidden w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-white/5 flex items-center justify-center text-white"
              >
            ...
            <div className="flex flex-col gap-8">
              ...
              {user && (
                <button
                  ...
                  className="text-xl font-black text-red-500 tracking-tighter text-left uppercase"
                >
                  Sign Out
                </button>
              )}
